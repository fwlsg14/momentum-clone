...
const LINKS = document.querySelector(".links"),
  LINK_LIST = LINKS.querySelector(".links-list"),
  LINKS_BTN = LINKS.querySelector("#links-btn"),
  NEWLINK_TAB = LINKS.querySelector(".new-link-tab"),
  NEWLINK_FORM = NEWLINK_TAB.querySelector("form"),
  NEWLINK_NAME = NEWLINK_FORM.querySelector("#new-link-name"),
  NEWLINK_URL = NEWLINK_FORM.querySelector("#new-link-url");

//모듈 변수
var links = links || {};
links.LinkList = links.LinkList || {};
...
links.checkLocalStorage = function () {
  console.log("로컬 스토리지에 저장된 데이터의 유효성 확인");
  let i = 0;
  let name;
  let URL;
  while (true) {
...
links.loadLocalStorage = function () {
  let space = links.checkLocalStorage().space;
...
links.createEditLinkTab = function (number) {
  ...
  let label2For = document.createAttribute("for");
  let label2Text = document.createTextNode("URL");
  label2For.value = `link-${number}-url`;
  label2.setAttributeNode(label2For);
  label2.appendChild(label2Text);
  let input2 = document.createElement("input");
  let input2Id = document.createAttribute("id");
  let input2Value = document.createAttribute("value");
  ...
  let input3Id = document.createAttribute("value");
  let input3Type = document.createAttribute("type");
  input3Id.value = `Edit`;
...
links.createLink = function (number, name, URL) {
  ...
  let anchorText = document.createTextNode(`${name}`);
  ...
  links.ButtonEvent[`detectButtonEventLink_${number}`] = function (number) {
    const CURRENT_LINK = LINK_LIST.querySelector(`#link-${number}`);
    const MORE_BUTTON = CURRENT_LINK.querySelector(`#link-${number}__btn`);
    const LINK_MORE = LINKS.querySelector(`#link-${number}__more`);
    const FORM = LINK_MORE.querySelector("form");
    const DELETE_BUTTON = LINK_MORE.querySelector(`#link-${number}-delete`);
  ...
  links.ButtonEvent[`editLink_${number}`] = function (event) {
    ...
    const LINK_MORE = LINKS.querySelector(`#link-${number}__more`);
    const FORM = LINK_MORE.querySelector("form");
    const EDIT_NAME = FORM.querySelector(`#link-${number}-name`);
    const EDIT_URL = FORM.querySelector(`#link-${number}-url`);
    let name = EDIT_NAME.value;
    let URL = EDIT_URL.value;
...
links.handleNewLinkSubmit = function (event) {
  event.preventDefault();
  hide(LINKS.querySelector(".new-link-tab"));
  let space = links.checkLocalStorage().space;
  let name = NEWLINK_NAME.value;
  let URL = NEWLINK_URL.value;
  NEWLINK_NAME.value = "";
  NEWLINK_URL.value = "";
...
links.createLinkList = function () {
  let amount = links.loadLocalStorage().amount;
  ...
  let linkId = document.createAttribute("id");
  ...
  let spanText = document.createTextNode("New Link");
...
links.toggleNewLinkTab = function () {
  toggleHidden(LINKS.querySelector(".new-link-tab"));
};
//버튼 클릭 감지
links.togglebutton = function () {
